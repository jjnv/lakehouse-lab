  ...
  return <div className="ent-page-stack">
    ...
    <section className="ent-focus-layout" aria-label="Prioridad de aprendizaje">
      <article className="ent-focus-card">
        ...
        <Link className="ent-primary-action" href={dashboard.nextActivity.href}>Continuar <span aria-hidden="true">→</span></Link>
      </article>
    ...
    <section className="ent-overview-grid" aria-label="Estado de la ruta">
      ...
      <article><span>Repasos pendientes</span><strong>{dashboard.reviews.length}</strong><Link href="/mi-aprendizaje#repasos">Abrir cola</Link></article>
      <article><span>Mejor Professional</span><strong>{dashboard.bestSimulatorScores.professional === null ? "—" : `${dashboard.bestSimulatorScores.professional}%`}</strong><Link href="/simulacro/professional">Practicar</Link></article>
    </section>

    <section className="ent-section" aria-labelledby="route-status-heading">
      <div className="ent-section-heading"><div><p className="ent-kicker">20 semanas · 100 horas</p><h2 id="route-status-heading">Tu avance, sin ruido.</h2></div><Link href="/mi-aprendizaje">Ver ruta completa</Link></div>
...
export function MyLearningV2() {
  ...
  return <div className="ent-page-stack">
    ...
    <section id="repasos" className="ent-section" aria-labelledby="reviews-heading"><div className="ent-section-heading"><div><p className="ent-kicker">Recuerdo espaciado</p><h2 id="reviews-heading">Repasos pendientes</h2></div><span>{dashboard.reviews.length}</span></div>
      {dashboard.reviews.length ? <div className="ent-review-list">{dashboard.reviews.map((review) => { const module = dashboard.modules.find((item) => item.id === review.moduleId); return <article key={`${review.moduleId}-${review.lessonId}`}><span>{module?.number}</span><div><b>{module?.short}</b><small>Intervalo {review.intervalDays} días · venció {review.dueOn}</small></div><Link href={`/curso/${module?.slug}?lesson=${review.lessonId}`}>Repasar</Link></article>; })}</div> : <div className="ent-empty"><strong>Todo al día</strong><p>Los próximos repasos aparecerán aquí cuando corresponda.</p></div>}
    </section>
  </div>;
}
...
function ModuleRow({ module, dashboard }: { module: ModuleSummary; dashboard: LearnerDashboard }) {
  ...
  return <article className={`ent-module-row ${progress?.completed ? "is-complete" : ""} ${progress?.unlocked ? "" : "is-preview"}`}>
    <span>{module.number}</span><div><small>{module.level}</small><h3>{module.title}</h3><p>{progress?.completedLessonIds.length ?? 0}/5 lecciones · {progress?.labAttested ? "laboratorio completado" : "laboratorio pendiente"} · {progress?.quizBestPercent === null || progress?.quizBestPercent === undefined ? "test pendiente" : `${progress.quizBestPercent}%`}</p><ProgressBar value={percent} label={`Progreso de ${module.title}`} /></div><div><b>{progress?.completed ? "Superado" : progress?.unlocked ? `${percent}%` : "Vista previa"}</b><Link href={`/curso/${module.slug}`}>{progress?.unlocked ? percent ? "Continuar" : "Empezar" : "Ver contenido"}</Link></div>
  </article>;
}
